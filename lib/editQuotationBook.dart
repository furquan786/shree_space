...
class EditQuotationBookui extends StatefulWidget {
  //const AddAnnuoncementui({Key key}) : super(key: key);
  final UserType type;
  String id;
  EditQuotationBookui({required this.type, this.id = "", Key? key})
      : super(key: key);

  @override
  State<EditQuotationBookui> createState() => _EditQuotationBookuiState();
}

class _EditQuotationBookuiState extends State<EditQuotationBookui> {
  List _children = [];
  int currentIndex = 2;
  bool isLoding = false;
                ...
                title: Row(
                  ...
                  height: 50,
                    ...
                    Container(
                        ...
                        child: Text("Edit Quotation Book"))
                  ],
                ),
              ),
...
class _HomeholderWidgetState extends State<HomePlaceholderWidget> {
  ...
  getQuaotationDetail() async {
    setLoding();
    final requestData = await API.quotationHistoryDetailList(widget.id);
    setLoding();
    if (requestData.status == 1) {
                                                                      ...
                                                                      child:
                                                                        ...
                                                                        child: ElevatedButton(
                                                                            ...
                                                                            onPressed: () async {
                                                                              Navigator.push(
                                                                                context,
                                                                                MaterialPageRoute(builder: (context) => UpdateQuotationItemUi(type: widget.type!, id: widget.id, item: quotationitemList[i])),
                                                                              );
                                                                            },
                                                                            child: Text(
                                                                              "Edit",
                                                                      ...
                                                                      child:
                                                                        ...
                                                                        child: ElevatedButton(
                                                                            ...
                                                                            onPressed: () async {
                                                                              ...
                                                                              final requestData = await API.deletequotationAPI(quotationitemList[i].id, quotationitemList[i].qot_id);
                                                                              if (requestData.status == 1) {
                                                                                setLoding();
                                                                                Navigator.push(
                                                                                  context,
                                                                                  MaterialPageRoute(
                                                                                      builder: (context) => EditQuotationBookui(
                                                                                            type: widget.type!,
                                                                                            id: widget.id,
                                                                                          )),
                                                                                );
              ...
              if (await DataConnectionChecker().hasConnection) {
                if (_fbKey.currentState!.validate()) {
                  if (selectedstate != null && selectedcity != null) {
                    ...
                    map["cust_uniq_code"] = customeruniquecodeController.text;
                    map["start_date"] = start_date;
                    map["end_date"] = end_date;
                    map["ocation"] = occasionController.text;
                    ...
                    map["items"] = quotationList.isNotEmpty
                        ? QuotationModel.fromListToJson(quotationList)
                        : "";
                    final data = await API.qutationeditAPI(map);
                    if (data.status == 1) {
                      setLoding();
