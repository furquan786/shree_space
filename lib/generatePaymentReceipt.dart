...
class GeneratePaymentReceiptui extends StatefulWidget {
  //const AddAnnuoncementui({Key key}) : super(key: key);
  final UserType type;
  String id;

  GeneratePaymentReceiptui({required this.type, this.id = "", Key? key})
      : super(key: key);

  @override
  State<GeneratePaymentReceiptui> createState() =>
      _GeneratePaymentReceiptuiState();
}

class _GeneratePaymentReceiptuiState extends State<GeneratePaymentReceiptui> {
  List _children = [];
  int currentIndex = 2;
  bool isLoding = false;
                  ...
                  height: 50,
                    ...
                    Expanded(
                      child: Container(
                          ...
                          child: Text("GENERATE PAYMENT RECEIPT",
                              style:
...
class _HomeholderWidgetState extends State<HomePlaceholderWidget> {
  ...
  File? _image1;
  var selectedstate, selectedcity, paymentMode;
  List<StateListModel> stateList = [];
  List<CityListModel> cityList = [];
  List<QuotationModel> quotationList = [];
  List<String> selectPaymentMode = ["Cash", "Cheque", "Online", "Settlement"];
  ...
  // getQuotationItemList() async {
  //   quotationitemList = [];
  //   setLoding();
  //   final requestData = await API.quotationItemDetailList(widget.id);
  //   setLoding();
  //   if (requestData.status == 1) {
  //     for (var i in requestData.result) {
  //       quotationitemList.add(QuotationModel.fromJson(i));
  //     }
  //     setState(() {});
  //   }
  // }

  getQuaotationDetail() async {
    setLoding();
    final requestData = await API.invoiceDetailApi(widget.id);
    setLoding();
    if (requestData.status == 1) {
      ocation = requestData.result[0]["ocation"];
      qot_id = requestData.result[0]["id"];
      qot_no = requestData.result[0]["qot_no"];
      ...
      state_n = requestData.result[0]["state_n"];
      city_n = requestData.result[0]["city_n"];
      city_pin = requestData.result[0]["city_pin"];
      address = requestData.result[0]["address"];
      logo = requestData.result[0]["lg_img"];
      b_name = requestData.result[0]["b_name"];
      inv_no = requestData.result[0]["inv_no"];
      rem_amt = requestData.result[0]["rem_amt"];
      total_amount = requestData.result[0]["amount"];
      if (start_date != "0000-00-00") {
        String formattedDate =
            DateFormat('dd-MM-yyyy').format(DateTime.parse(start_date));
        dateController.text = formattedDate;
        stratdate = DateTime.parse(start_date);
      }
      ...
      brandnameController.text = b_name;
      addressController.text = address;
      citypinController.text = city_pin;
      occasionController.text = ocation;
                ...
                child: Padding(
                  ...
                  child: Column(
                      ...
                      children: [
                        ...
                        paymentModeDropDown(
                            "Select Payment Mode", selectPaymentMode, (newVal) {
                          if (newVal != paymentMode) {
                            paymentMode = newVal;
                            setState(() {});
                          }
                        }, paymentMode),
  ...
  Widget paymentModeDropDown(String hint, List<String> list,
      Function(String? val) onChange, String value) {
    return SizedBox(
      child: DropdownButtonHideUnderline(
        child: DropdownButtonFormField(
              ...
              if (await DataConnectionChecker().hasConnection) {
                if (_fbKey.currentState!.validate()) {
                  if (selectedstate != null && selectedcity != null) {
                    ...
                    map["type"] = paymentMode;
                    map["amount"] = amountController.text;
                    map["remark"] = remarkController.text;
                    map["rem_amt"] = rem_amt;

                    print("form data is = $map");
                    final data = await API.paymentReceiptApi(map);
                    if (data.status == 1) {
                      setLoding();
