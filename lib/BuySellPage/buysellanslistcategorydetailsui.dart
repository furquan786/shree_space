...
import 'buysell_searchscreenui.dart';
...
class BuySelllistcategorydetailsui extends StatefulWidget {
  String index, title;
  final UserType type;
  BuySelllistcategorydetailsui(
      {required this.index, required this.title, required this.type, Key? key})
      : super(key: key);

  @override
  State<BuySelllistcategorydetailsui> createState() =>
      _BuySelllistcategorydetailsuiState();
}

class _BuySelllistcategorydetailsuiState
    extends State<BuySelllistcategorydetailsui> {
  List _children = [];
  int currentIndex = 2;
  bool isLoding = false;
        ...
        appBar: AppBar(
          ...
          actions: [
            IconButton(
              ...
              onPressed: () {
                Get.to(BuySellSearchscreenui(type: widget.type));
              },
            ),
          ],
        ),
...
class _HomeholderWidgetState extends State<HomePlaceholderWidget> {
  ...
  getQueAnsList() async {
    ...
    setLoding();
    final requestData = await API.buysellCategoryDetailList(widget.index!);
    print(requestData);
    setLoding();
