...
import 'package:shreegraphic/BuySellPage/rentselectbusellcategoryui.dart';
...
class BuySellRentPage extends StatefulWidget {
  final UserType type;

  BuySellRentPage({required this.type, Key? key}) : super(key: key);

  @override
  State<BuySellRentPage> createState() => _BuySellRentPageState();
}

class _BuySellRentPageState extends State<BuySellRentPage>
...
class _BuySellListWidgetState extends State<BuySellListWidget> {
  ...
  getBuysellList() async {
    buysellList = [];
    setLoding();
    final requestData = await API.rentbuysellList();
    setLoding();
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: ElevatedButton(
                              ...
                              onPressed: () async {
                                Get.to(RentSelectbusellcategoryui(
                                    type: widget.type));
                              },
                              child: Text(
                                "Select Category",
...
class _SellHistoryWidgetState extends State<SellHistoryWidget> {
  ...
  getOrderList() async {
    ...
    myorderList = [];
    setLoding();
    final requestData = await API.myordersellRentHistoryList(sell_id);
    setLoding();
    if (requestData.status == 1) {
      for (var i in requestData.result) {
        myorderList.add(MyOrderListModel.fromJson(i));
