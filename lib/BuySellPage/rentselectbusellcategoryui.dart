...
import 'package:shreegraphic/BuySellPage/rentbuysell_cat_searchscreenui.dart';
import 'package:shreegraphic/BuySellPage/rentbuysellanslistcategorydetailsui.dart';
...
class RentSelectbusellcategoryui extends StatefulWidget {
  final UserType type;

  RentSelectbusellcategoryui({required this.type, Key? key}) : super(key: key);
  // const Selectbusellcategoryui({Key key}) : super(key: key);

  @override
  State<RentSelectbusellcategoryui> createState() =>
      _RentSelectbusellcategoryuiuiState();
}

class _RentSelectbusellcategoryuiuiState
    extends State<RentSelectbusellcategoryui> {
  List _children = [];
  int currentIndex = 2;
  bool isLoding = false;
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () {
              Get.to(RentBuySellCatSearchscreenui(type: widget.type));
            },
          ),
        ],
      ),
...
class _HomeholderWidgetState extends State<HomePlaceholderWidget> {
  ...
  getNewList() async {
    buysellList = [];
    setLoding();
    final requestData = await API.rentbuysellCategoryList();
    setLoding();
