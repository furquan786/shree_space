...
class MyCatImagesui extends StatefulWidget {
  String index, folderName, user_id;
  final UserType type;
  ...
  @override
  State<MyCatImagesui> createState() => _MyCatImagesuiState();
}

class _MyCatImagesuiState extends State<MyCatImagesui> {
  bool isLoding = false;
  ...
  getImageList() async {
    ...
    final requestData =
        await API.mycatimageListAPI(user_ID, widget.index, widget.folderName);
    setLoding();
    if (requestData.status == 1) {
      for (var i in requestData.result) {
  ...
  /*getCustImageList() async {
    myimageList = [];
    setLoding();
    final requestData =
        await API.mycatimageListAPI(widget.user_id, widget.index, widget.folderName);
    setLoding();
    if (requestData.status == 1) {
      for (var i in requestData.result) {
        myimageList.add(MyCatIamgeListModel.fromJson(i));
...
*/
                        ...
                        ? PopupMenuButton(
                            ...
                            return [
                              ...
                              PopupMenuItem<int>(
                                value: 2,
                                child: _buildPopupMenuItem(
                                    'Delete', Icons.delete, 2),
                              ),
                              PopupMenuItem<int>(
                                value: 3,
                                child: _buildPopupMenuItem(
                                    'Share For Selection', Icons.share, 3),
                              ),
                            ];
                          }, onSelected: (value) {
                            if (value == 0) {
                        ...
                        child: ElevatedButton(
                          ...
                          onPressed: () async {
                            if (_formKey.currentState!.validate()) {
                              ...
                              setLoding();
                              final requestData =
                                  await API.addfolderSelecationAPI(
  ...
  _onMenuItemSelected3() {
    getUploadPhotos(widget.index);
  }

  void getUploadPhotos(String id) {
    showDialog(
        context: context,
        builder: (context) {
  ...
  void _showPickermultiple(context, _setState) {
    showModalBottomSheet(
  ...
  _imgFromDocument1(StateSetter _setState) async {
    try {
  ...
  _imgFromCamera1(StateSetter _setState) async {
  ...
  Future<void> selecteApi(List<String> selectedlist) async {
    setLoding();
    final requestData = await API.selectedImageAPI(
      selectedlist,
    );
    if (requestData.status == 1) {
      setLoding();
        ...
        Navigator.push(
          ...
          MaterialPageRoute(
              builder: (context) => MyCatImagesui(
                  ...
                  folderName: widget.item.folder_name,
                                                                      ...
                                                                      if (_formKey
                                                                        ...
                                                                        if (requestData.status ==
                                                                          ...
                                                                          Navigator
                                                                              .push(
                                                                            context,
                                                                            MaterialPageRoute(builder: (context) => MyCatImagesui(index: widget.catindex, folderName: widget.foldname, user_id: widget.item.user_id, type: widget.type)),
                                                                          );
                                                                        } else {
                                                                          setLoding();
                                                                          Fluttertoast.showToast(
...
enum SampleItem { share, move, delete }
...
class GridItem extends StatefulWidget {
  ...
  List<MemberListModel> memberList;
  List<LABListModel> labList = [];
  String catindex, user_id, foldname;
  final UserType type;
...
class _GridItemState extends State<GridItem> {
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      onTap: () async {
        String user_ID;
        user_ID = await ShareManager.getUserID();
        if (isSelected == true) {
          setState(() {
            isSelected = !isSelected;
            widget.isSelected(!isSelected);
  ...
  Future<void> unselecteApi(List<String> selectedlist) async {
    setLoding();
    final requestData = await API.unselectedImageAPI(
      selectedlist,
    );
    if (requestData.status == 1) {
      setLoding();
