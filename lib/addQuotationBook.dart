...
class AddQuotationBookui extends StatefulWidget {
  //const AddAnnuoncementui({Key key}) : super(key: key);
  final UserType type;

  AddQuotationBookui({required this.type, Key? key}) : super(key: key);

  @override
  State<AddQuotationBookui> createState() => _AddQuotationBookuiState();
}

class _AddQuotationBookuiState extends State<AddQuotationBookui> {
  List _children = [];
  int currentIndex = 2;
  bool isLoding = false;
...
class _HomeholderWidgetState extends State<HomePlaceholderWidget> {
  ...
  var logo, selectedEndDate, selectedStartDate;
  File? _image1;
  String selectedstate = "", selectedcity = "", daysbetweeen = "";
      ...
      child: Row(
        children: [
          ...
          MaterialButton(
            ...
            onPressed: () {
              print("qutation list is = ${quotationList}");
              Navigator.push(
                context,
                MaterialPageRoute(
  ...
  Future<Null> _selectDate2(TextEditingController selectedDate) async {
    ...
    if (stratdate != null && enddate != null) {
      daysbetweeen = daysBetween(stratdate, enddate).toString();
      print(daysbetweeen);
    }
  }
              ...
              if (await DataConnectionChecker().hasConnection) {
                if (_fbKey.currentState!.validate()) {
                  if (selectedstate != null &&
                    ...
                    map["start_date"] = selectedStartDate;
                    map["end_date"] = selectedEndDate;
                    map["ocation"] = occasionController.text;
                    /*map["side"] = selectedSide ?? "";*/
                    ...
                    map["requiremet"] = requirmentController.text;
                    map["remark"] = remarkController.text;
                    map["logo"] = profilelist;
                    map["b_name"] = brandnameController.text;
                    map["items"] = QuotationModel.fromListToJson(quotationList);
                    final data = await API.qutationaddAPI(map);
                    if (data.status == 1) {
                      setLoding();
