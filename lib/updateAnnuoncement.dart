...
class UpdateAnnuoncementui extends StatefulWidget {
  String index;
  final UserType type;
  UpdateAnnuoncementui({this.index = '', required this.type, Key? key})
      : super(key: key);

  @override
  State<UpdateAnnuoncementui> createState() => _UpdateAnnuoncementuiState();
}

class _UpdateAnnuoncementuiState extends State<UpdateAnnuoncementui> {
  List _children = [];
  int currentIndex = 2;
  bool isLoding = false;
...
class _HomeholderWidgetState extends State<HomePlaceholderWidget> {
  ...
  getAnnumentList() async {
    ...
    setLoding();
    if (requestData.status == 1) {
      selectedOccasion = requestData.result[0]["occasion"];
      selectedSide = requestData.result[0]["side"];
      selectedCamera = requestData.result[0]["camera"];
      selectedTime = requestData.result[0]["time"];
      ...
      anno_desc = requestData.result[0]["anno_desc"];
      daysbetweeen = requestData.result[0]["days"];
      occ_loc = requestData.result[0]["occ_loc"];
      custom_location = requestData.result[0]["custom_location"];
      cast = requestData.result[0]["cast"];
      annodescController.text = anno_desc;
      locationController.text = occ_loc;
      customlocationController.text = custom_location;
      castController.text = cast;
      ...
      setState(() {
        ...
        for (int i = 0; i < cameraList.length; i++) {
          if (selectedCamera == cameraList[i]) {
            index2 = i;
          }
        }
                ...
                child: Padding(
                  ...
                  child: Column(
                      ...
                      children: [
                        ...
                        /*timeDropDownMenu("Select Time", timeList, (newVal) {
                          if (index3 != null) {
                            if (newVal != timeList[index3]) {
                              index3 = null;
                              timeList[index3] = newVal;
                              setState(() {});
                            }
                          } else {
                            if (newVal != selectedTime) {
    ...
    return SizedBox(
      child: DropdownButtonHideUnderline(
        child: DropdownButtonFormField(
            ...
            value: index3 != null ? timeList[index3] : value,
            hint: Text(
              hint,
    ...
    return SizedBox(
      child: DropdownButtonHideUnderline(
        child: DropdownButtonFormField(
            ...
            value: index2 != null ? cameraList[index2] : value,
            hint: Text(
              hint,
              style: TextStyle(color: Colors.grey, fontSize: 12),
            ),
              ...
              if (await DataConnectionChecker().hasConnection) {
                if (_fbKey.currentState!.validate()) {
                  if (selectedOccasion != null &&
                    ...
                    String s1 = work.join(', ');
                    final requestData = await API.editAnnouncementApi(
                      widget.index,
                      member_name,
                      member_mobile,
                      selectedOccasion,
