...
import 'locationAllDetailui.dart';
import 'locationMyDetailui.dart';
...
class LocationForshoot extends StatefulWidget {
  final UserType type;

  LocationForshoot({required this.type, Key? key}) : super(key: key);

  @override
  State<LocationForshoot> createState() => _LocationForshootState();
}

class _LocationForshootState extends State<LocationForshoot>
            ...
            title: Row(
              ...
              //width: 150,
                ...
                Container(
                    ...
                    child: const Text("Location For Shoot"))
              ],
            ),
          ),
          body: Stack(children: [
            Container(
                ...
                child: ContainedTabBarView(
                  ...
                  views: [
                    AllLocationWidget(type: widget.type),
                    PaidLocationWidget(type: widget.type),
                    MYLocationWidget(type: widget.type),
                  ],
...
class AllLocationWidget extends StatefulWidget {
  UserType type;

  AllLocationWidget({required this.type, Key? key}) : super(key: key);

  @override
  State<AllLocationWidget> createState() => _AllLocationWidgetState();
}

class _AllLocationWidgetState extends State<AllLocationWidget> {
  bool isLoding = false;
  ...
  getLocationList() async {
    ...
    setLoding();
    final requestData = await API.alllocationshootListAPI();
    setLoding();
                              ...
                              child: ListView.separated(
                                  ...
                                  itemBuilder: (BuildContext context, int i) {
                                    return GestureDetector(
                                      onTap: () {
                                        Get.to(LocationAllDetailhistoryui(
...
class PaidLocationWidget extends StatefulWidget {
  UserType type;

  PaidLocationWidget({required this.type, Key? key}) : super(key: key);
  @override
  State<PaidLocationWidget> createState() => _PaidLocationWidgetState();
}

class _PaidLocationWidgetState extends State<PaidLocationWidget> {
  bool isLoding = false;
  List<LocforShootModel> locforshootList = [];
  getLocationList() async {
    locforshootList = [];
...
class MYLocationWidget extends StatefulWidget {
  ...
  MYLocationWidget({required this.type, Key? key}) : super(key: key);
  @override
...
class _MYLocationWidgetState extends State<MYLocationWidget> {
  ...
  getLocationList() async {
    ...
    String user_id = await ShareManager.getUserID();
    setLoding();
    final requestData = await API.locationListAPI(user_id);
    setLoding();
    if (requestData.status == 1) {
      for (var i in requestData.result) {
        locforshootList.add(LocforShootModel.fromJson(i));
                          ...
                          : Padding(
                              ...
                              child: ListView.separated(
                                  ...
                                  itemBuilder: (BuildContext context, int i) {
                                    return GestureDetector(
                                      onTap: () {
                                        Get.to(LocationMyDetailhistoryui(
                                            index: locforshootList[i]
                                                .id
                                                .toString(),
                                            type: widget.type));
