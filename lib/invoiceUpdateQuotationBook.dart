...
class InvoiceQuotationBookui extends StatefulWidget {
  //const AddAnnuoncementui({Key key}) : super(key: key);
  final UserType type;
  String id;

  InvoiceQuotationBookui({required this.type, this.id = "", Key? key})
      : super(key: key);

  @override
  State<InvoiceQuotationBookui> createState() => _InvoiceQuotationBookuiState();
}

class _InvoiceQuotationBookuiState extends State<InvoiceQuotationBookui> {
  List _children = [];
  int currentIndex = 2;
  bool isLoding = false;
                ...
                if (_fbKey.currentState!.validate()) {
                  if (selectedstate != null && selectedcity != null) {
                    ...
                    map["new_items"] = quotationList.isNotEmpty
                        ...
                        : "";
                    map["sel_items"] = item_idlist.isNotEmpty
                        ? QuotationModel.fromListToJson(item_idlist)
                        : "";
                    final data = await API.qutationinvoiceAPI(map);
                    if (data.status == 1) {
                      setLoding();
